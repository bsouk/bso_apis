/**
 * Run IAP Test with Existing Plan
 * Uses an existing plan from database
 */
...
const PLAN_ID = process.argv[3] || null; // Will find supplier monthly plan if not provided
...
async function runTest() {
  try {
    ...
    let planData;
    if (PLAN_ID) {
      planData = await plan.findOne({ plan_id: PLAN_ID, status: 'active' });
    } else {
      // Find supplier monthly plan
    ...
    if (!planData) {
      ...
      console.log('\n💡 Available supplier plans:');
      const allPlans = await plan.find({ type: 'supplier', status: 'active' });
      allPlans.forEach(p => {
        console.log(`   - ${p.plan_id}: ${p.plan_name} (${p.interval})`);
      });
      process.exit(1);
    }
